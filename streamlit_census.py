...
query2 = "SELECT district,sum(Literate_Male),sum(Literate_Female) FROM census_1.census GROUP BY district"
...
query3 = """SELECT district,ROUND(SUM(Male_Workers) * 100.0 / SUM(Workers)) AS male_percentage,
...
query4 = """SELECT district,sum(LPG_or_PNG_Households)from census_1.census GROUP BY district"""
...
query6 ="""SELECT district, SUM(Households_with_Internet) as SUM_Households_with_Internet FROM census_1.census group by District"""
...
query11 ="""select State_or_UT, sum(Households) from census_1.census GROUP BY State_or_UT"""
...
query12 ="""select State_or_UT,SUM(Having_latrine_facility_within_the_premises_Total_Households) AS count_Latrine_within_premise from census_1.census
...
query13 ="""select State_or_UT,SUM(Households)/COUNT(Households) AS Average_household_size
FROM census_1.census
GROUP BY State_or_UT"""
...
query14 ="""select State_or_UT,
SUM(Ownership_Owned_Households) AS count_owned_house ,
